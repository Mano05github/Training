...
inner join enrollments e where s.city = 'hyderabad';
...
select s.student_name, sum(e.fee) as total_fees from students s 
inner join enrollments e on s.student_id = e.student_id
group by s.student_name;

-- exercise 10

select s.student_name, count(e.enrollment_id) as course_count from students s
inner join enrollments e on s.student_id = e.student_id
...
select trainer, sum(fee) as total_fees from enrollments group by trainer having sum(fee)>10000;


-- exercise 12

select city, count(student_id) as student_count from students group by city having count(student_id) > 1;
...
having sum(e.fee) > 5000 order by total_fee_paid desc;